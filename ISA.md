...
4 регистра размером в 16 бит, общего назначения:
...
3 регистра(16 бит):
* `PC` (Program counter). Это адресс в памяти который будет выполнен процесором следующим. Номер 4.
* `SP` (Stack pointer). Это указатель на вершину стека. Номер 5.
* `Flags`. Это регистер флагов. Номер 6.
  1. Бит 0 -- это флаг нуля (`Zero flag`). Он установлен если результат прошлой арефметической операции был ноль.
  2. Бит 1 -- это флаг переноса (`Carry flag`). Он устонавливается если во время выполнения арефметической операции был перенос или заем.
  3. Остальное зарезервировано.
...
## Команды
### Формат команд
* 16 битный код команды. Состоит из:
  * Базовый код(8 бит)
  * Номер регистра 1(4 бит). Не для всех команд, там где неиспользуется -- 0
  * Номер регистра 2(4 бит). Не для всех команд, там где неиспользуется -- 0
* 16 битный Аргумент. Не для всех команд, там где неиспользуется отсутствует

Для каждой команды процесор делает следущее(пример):
* Берется код команды: `0x0123`
* `23` делятся на `R1`(Будет равен 3) и `R2`(Будет равен 2)
...
| 0x06        | SHL R1      | Нет      | ZF, CF           | R1                    | CF = R1\[15\]; R1 <<= 1                       |
